...
module.exports = (baseProvider, options, app) => {
  ...
  const { getTriggersScript, hydrateTriggers } =
    require("./helpers/triggerHelper")({
      _,
      wrap,
      assignTemplates,
      templates,
  ...
  return {
    createDatabase(modelData) {
      ...
      const { locale, collate, characterClassification } =
        getLocaleProperties(modelData);
    ...
    createSchema({ schemaName, ifNotExist, comments, udfs, procedures }) {
      ...
      const createProceduresStatement = getProceduresScript(
        schemaName,
        procedures
      );
    ...
    createTable(
      ...
      const keyConstraintsValue = partitionOf
        ? keyConstraintsString?.slice(1)
        : keyConstraintsString;

      const dividedForeignKeys = divideIntoActivatedAndDeactivated(
        foreignKeyConstraints,
        (key) => key.statement
      );
      ...
      const template = partitionOf
        ? templates.createTablePartitionOf
      ...
      return commentIfDeactivated(
        [tableStatement, createTriggerStatements]
          .map(_.trim)
          .join("\n\n")
          .trim() + "\n",
        { isActivated }
    ...
    convertColumnDefinition(columnDefinition) {
      const type = replaceTypeByVersion(
        columnDefinition.type,
        columnDefinition.dbVersion
      );
      ...
      const collation = columnDefinition.collationRule
        ? ` COLLATE "${columnDefinition.collationRule}"`
        : "";
    ...
    createIndex(tableName, index, dbData, isParentActivated = true) {
      ...
      const using = index.index_method
        ? ` USING ${_.toUpper(index.index_method)}`
        : "";
      const dbVersion = getDbVersion(_.get(dbData, "dbVersion", ""));
      const nullsDistinct =
        isUnique && index.nullsDistinct && dbVersion >= 15
          ? `\n ${index.nullsDistinct}`
          : "";
      ...
      const keys = getIndexKeys(
        index.index_method === "btree"
          ? index.columns
          : _.map(index.columns, (column) =>
              _.omit(column, "sortOrder", "nullsOrder")
            ),
        isParentActivated
      );
    ...
    createCheckConstraint(checkConstraint) {
      return assignTemplates(templates.checkConstraint, {
        name: checkConstraint.name
          ? `CONSTRAINT ${wrapInQuotes(checkConstraint.name)}`
          : "",
        expression: _.trim(checkConstraint.expression).replace(
          /^\(([\s\S]*)\)$/,
          "$1"
        ),
    ...
    createForeignKey(
      ...
      const foreignKeyStatement = assignTemplates(templates.createForeignKey, {
        ...
        foreignTable: getNamePrefixedWithSchemaName(
          foreignTable,
    ...
    createView(viewData, dbData, isActivated) {
      const viewName = getNamePrefixedWithSchemaName(
        viewData.name,
        viewData.schemaName
      );
      ...
      let columnsAsString = columns
        .map((column) => column.statement)
        .join(",\n\t\t");

      if (!deactivatedWholeStatement) {
        const dividedColumns = divideIntoActivatedAndDeactivated(
          columns,
        ...
        const deactivatedColumnsString = dividedColumns.deactivatedItems.length
          ...
          : "";
        columnsAsString =
          dividedColumns.activatedItems.join(",\n\t\t") +
      ...
      const check_option = viewData.viewOptions?.check_option
        ...
        : "";
      const security_barrier = viewData.viewOptions?.security_barrier
        ? `security_barrier`
        : "";
      ...
      return (
        [createViewScript, createTriggersStatements]
          .map(_.trim)
          .join("\n\n")
          .trim() + "\n"
      );
    ...
    hydrateColumn({
      ...
      const collationRule = _.includes(
        ["char", "varchar", "text"],
        columnDefinition.type
      )
      ...
      const timePrecision = _.includes(timeTypes, columnDefinition.type)
        ? jsonSchema.timePrecision
        : "";
      const timezone = _.includes(timeTypes, columnDefinition.type)
        ? jsonSchema.timezone
        : "";
      const intervalOptions =
        columnDefinition.type === "interval" ? jsonSchema.intervalOptions : "";
      ...
      return {
        ...
        comment:
          ...
          definitionJsonSchema.description,
        isActivated: columnDefinition.isActivated,
    ...
    hydrateJsonSchemaColumn(jsonSchema, definitionJsonSchema) {
      if (
        !jsonSchema.$ref ||
        _.isEmpty(definitionJsonSchema) ||
        isNotPlainType(definitionJsonSchema)
      ) {
        return jsonSchema;
      }
    ...
    hydrateTable({ tableData, entityData, jsonSchema }) {
      ...
      const parentTables = _.chain(detailsTab.inherits)
        .map(({ parentTable }) =>
          _.get(tableData, `relatedSchemas[${parentTable}]`, "")
        )
        .compact()
      ...
      const compositePartitionKey = keyHelper.getKeys(
        partitioning.compositePartitionKey,
        jsonSchema
      );
      const partitionParent = _.get(
        tableData,
        `relatedSchemas[${detailsTab.partitionOf}]`
      );
      const partitionOf = partitionParent
        ? getNamePrefixedWithSchemaName(
            partitionParent.collectionName,
            partitionParent.bucketName
          )
        : "";
    ...
    hydrateView({ viewData, entityData, relatedSchemas }) {
      ...
      return {
        ...
        checkTestingScope: detailsTab.withCheckOption
          ? detailsTab.checkTestingScope
          : "",
