	...
	({ columns, collection }) => {
		return (columns || [])
			.map(column => {
				return {
					...column,
					name: getColumnNameById({ _ })({ columnId: column.keyId, collection }),
				};
			})
 ...
 * }) => boolean}
 * */
const areIndexesDifferent =
	({ _ }) =>
	({ oldIndex, newIndex, indexPropertiesToCompare }) => {
...
const getCreateIndexScriptDto =
	({ _, ddlProvider }) =>
	({ index, collection, additionalDataForDdlProvider }) => {
 ...
 * }) => Array<AlterScriptDto>}
 * */
const getAlterIndexScriptDtos =
	({ _, ddlProvider }) =>
	({ newIndex, oldIndex, additionalDataForDdlProvider }) => {
		...
		const shouldUpdateStorageParams = !_.isEqual(
			newIndex.index_storage_parameter,
			oldIndex.index_storage_parameter,
		);
