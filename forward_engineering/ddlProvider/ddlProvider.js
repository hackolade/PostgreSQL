  ...
  return {
    ...
    createTable(
      ...
      const dividedKeysConstraints = divideIntoActivatedAndDeactivated(
        keyConstraints.filter(({ errorMessage }) => !errorMessage).map(createKeyConstraint(templates, isActivated)),
        key => key.statement,
      );
      const constraintWarnings = getConstraintsWarnings(keyConstraints.filter(({ errorMessage }) => errorMessage));
      ...
      return commentIfDeactivated([tableStatement, createTriggerStatements].map(_.trim).join('\n\n').trim() + '\n', {
        isActivated,
      });
    },
    ...
    createForeignKeyConstraint(
      ...
      const foreignKeyStatement = assignTemplates(templates.createForeignKeyConstraint, {
        ...
        deferrableConstraintCheckTime:
          deferrable === 'DEFERRABLE' && deferrableConstraintCheckTime ? ` ${deferrableConstraintCheckTime}` : '',
      });
    ...
    createView(viewData, dbData, isActivated) {
      ...
      return commentIfDeactivated([createViewScript, createTriggersStatements].map(_.trim).join('\n\n').trim() + '\n', {
        isActivated: !deactivatedWholeStatement,
      });
    },
