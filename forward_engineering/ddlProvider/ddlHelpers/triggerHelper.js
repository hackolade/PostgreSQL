...
const { wrap, getNamePrefixedWithSchemaName, commentIfDeactivated } = require('../../utils/general');
	...
	return trigger.triggerEvents
		?.map(event => {
			...
			const activatedKeys = _.chain(trigger.triggerUpdateColumns)
				...
				.map(({ name }) => name)
				.join(', ')
				.thru(activatedStr => (activatedStr ? `OF ${activatedStr}` : activatedStr))
				.value();
			const deactivatedKeys = _.chain(trigger.triggerUpdateColumns)
				...
				.join(', ')
				.thru(deactivatedStr =>
					!activatedKeys && Boolean(deactivatedStr) ? `OF ${deactivatedStr}` : deactivatedStr,
				)
				.value();
